...
async function exactHorizon(taskId) {
  const { rows } = await substrate.query(
    `SELECT ownership_valid_until FROM cell.work_claim WHERE task_id = $1 AND state = 'active'`, [taskId]);
  return String(rows[0].ownership_valid_until.toISOString());
}

test('R1: while the cage shows life, the host NEVER touches the claim horizon', async () => {
  ...
  assert.equal(await exactHorizon('alive'), before,
    'session/process liveness must not extend organizational ownership — exact equality');
});
