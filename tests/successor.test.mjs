...
// Successor is runtime-only: the standalone provider refuses it TYPED (proven in
// runtime-provider.test.mjs). On a machine with no monorepo checkout, OR a checkout whose
// oathe-runtime symlink is missing (Finding 1 — `npm run link-runtime` was never run), skip
...
const RUNTIME_PRESENT = cagePresent && new OatheRuntimeProvider({ paths }).probe().ok;
const skip = !RUNTIME_PRESENT && 'oathe runtime not on this machine — successor is runtime-only';
