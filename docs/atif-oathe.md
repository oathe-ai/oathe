...
| tool call | `executions` | codex, from the exec source at call-start, completed by the item stream | the ledger of what ran INSIDE the call — every inner `tools.*` call the source names (`tool`, and the `arguments` it states), completed by its item (a command's `command` and `exit_code`; an MCP act's whole `arguments` and `status`); an entry whose item has not landed yet is `pending` (commands and MCP acts — the tools whose items complete them). A cell whose single inner call ran one command carries no ledger: that command IS the call, and its exit code is the result's. On the record the moment the cell is written — a blocking act inside a running cell is visible to its own verification (2026-09-04) |
| observation result | `exit_code` / `files_changed` | codex, from the item stream | the structural exit code (exactly one command), or the files a patch changed |
