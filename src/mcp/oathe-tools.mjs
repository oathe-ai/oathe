...
export function createOatheTools({ client, identity, workspace, executionActor, successor, config, verifier }) {
  ...
  return {
    ...
    async oathe_board({ all = false } = {}) {
      ...
      const { rows } = await client.query(
        `SELECT task_id, objective, origin, state, principal_id, contract_ref, settled_at, lease_until,
                last_progress, last_progress_at FROM (
           SELECT DISTINCT ON (t.task_id)
             ...
             LEFT JOIN LATERAL (
                 SELECT proposition, asserted_at FROM cell.agent_statement s
                  WHERE s.org_id = t.org_id AND s.task_id = t.task_id
                    AND s.work_claim_id = w.work_claim_id
                    AND s.statement_type = 'progress' AND s.subject_ref NOT LIKE 'trace:%'
