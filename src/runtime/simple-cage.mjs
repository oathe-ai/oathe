...
// (never merged) and the cage itself stamps FIRIA_EXECUTION_ATTEMPT_ID; the child leads a
...
export function spawnCaged({ unit, env, cmd, args, cwd, stdio, graceMs = DEFAULT_GRACE_MS }) {
  ...
  const child = spawn(cmd, args, {
    ...
    env: { ...env, FIRIA_EXECUTION_ATTEMPT_ID: attemptId }, // REPLACED env + the cage's own stamp
  });
